...
class SystemResponse(object):
    ...
    def add_accelerometer(self, resonance_frequency, gain, sensitivity=1, damping=0.707):
        resp = accelerometer_response(
            resonance_frequency,
            gain,
    ...
    def add_digitizer(
        self, max_voltage=2.5, resolution_bit=24, stage_gain_frequency=1, digitizer_name=None
    ):
    ...
    def response(self):
        ...
        return Response(instrument_sensitivity=i_s, response_stages=self.response_stages)


def geophone_response(
    resonance_frequency,
    gain,
    damping=0.707,
    output_resistance=np.inf,
...
def accelerometer_response(
    resonance_frequency, gain, sensitivity=1, stage_sequence_number=0, damping=0.707
):
    i_s = InstrumentSensitivity(
        sensitivity,
        resonance_frequency,
    ...
    pzr = PolesZerosResponseStage(
        stage_sequence_number, 1, 14, "M/S/S", "V", "LAPLACE (RADIANT/SECOND)", 1, [], paz["poles"]
    )
...
class Inventory(inventory.Inventory):
    ...
    @classmethod
    def from_obspy_inventory_object(
        cls, obspy_inventory, xy_from_lat_lon=False, input_projection=4326, output_projection=None
    ):
...
class Network(inventory.Network):
    ...
    @classmethod
    def from_obspy_network(
        cls, obspy_network, xy_from_lat_lon=False, input_projection=4326, output_projection=None
    ):
    ...
    def get_grid_extent(self, padding_fraction=0.1, ignore_stations=[], ignore_sites=[]):
        """
...
class Station(inventory.Station):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for key in kwargs_with_extra:
            if key in self.extra_keys:
                self.extra[key] = AttribDict({"value": kwargs_with_extra[key], "namespace": ns})
    ...
    @classmethod
    def from_obspy_station(
        cls, obspy_station, xy_from_lat_lon=False, output_projection=None, input_projection=4326
    ):
        ...
        if xy_from_lat_lon:
            if (stn.latitude is not None) and (stn.longitude is not None):
                ...
                logger.warning(
                    f"Latitude or Longitude are not" f"defined for station {obspy_station.code}."
                )
    ...
    def x(self):
        if self.extra:
            if self.extra.get("x", None):
                return float(self.extra.x.value)  # obspy inv_read converts everything
                # in extra to str
            else:
    ...
    def loc(self):
        if self.extra:
            if (
                self.extra.get("x", None)
                and self.extra.get("y", None)
                and self.extra.get("z", None)
            ):
    ...
    def __str__(self):
        ...
        ret = (
            f"\tStation {self.historical_code}\n"
        ...
        ret = ret.format(
            ...
            alternate_code="Alternate Code: %s " % self.alternate_code
            if self.alternate_code
            else "",
            historical_code="Historical Code: %s " % self.historical_code
            if self.historical_code
            else "",
            x=x,
            y=y,
            z=z,
...
class Site:
    ...
    def __init__(self, station, channels):
        ...
        if len(np.unique(location_codes)) > 1:
            logger.error("the channels in the channel list should have a" "unique location code")
            raise KeyError
...
class Channel(inventory.Channel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for key in kwargs_with_extra:
            if key in self.extra_keys:
                self.extra[key] = {"value": kwargs_with_extra[key], "namespace": ns}

    @classmethod
    def from_obspy_channel(
        cls, obspy_channel, xy_from_lat_lon=False, output_projection=None, input_projection=4326
    ):
        cha = cls(
            obspy_channel.code,
            obspy_channel.location_code,
        ...
        if hasattr(obspy_channel, "extra"):
            for key in cha.extra_keys:
                if key not in obspy_channel.__dict__["extra"].keys():
                    ...
                    cha.__dict__["extra"][key] = obspy_channel.__dict__["extra"][key]
    ...
    def __repr__(self):
        ret = (
            f"Channel {self.code}, Location {self.location_code}\n"
    ...
    def __encode_coordinates__(value):
        if not isinstance(value, Coordinates):
            raise TypeError(f"the value provided must be of instance " f"{type(Coordinates)}")
    ...
    def orientation_vector(self):
        ...
        east = np.sin(self.azimuth * np.pi / 180) * np.cos(self.dip * np.pi / 180)
        north = np.cos(self.azimuth * np.pi / 180) * np.cos(self.dip * np.pi / 180)
...
def load_from_excel(file_name) -> Inventory:
    ...
    for code in stn_codes:
        ...
        station["long_name"] = "{}.{}.{:02d}".format(
            row["network__code"], row["station_code"], row["location_code"]
        )
        ...
        renamed_keys = {"sensor_code", "location_x", "location_y", "location_z", "name"}
...
def read_inventory(
    ...
    **kwargs,
    ...
    if xy_from_lat_lon and (output_projection is None):
        raise ValueError(
            "the output projection is needed for conversion" "from latitude-longitude to UTM"
        )
    ...
    if (format not in ENTRY_POINTS["inventory"].keys()) or (format.upper() == "STATIONXML"):
        obspy_inv = inventory.read_inventory(path_or_file_object, format=format, *args, **kwargs)
        ...
        read_format = load_entry_point(
            format_ep.dist.key, "obspy.io.%s" % format_ep.name, "readFormat"
        )
        ...
        return expand_input_format_compatibility(read_format(path_or_file_object, **kwargs))
...
read_inventory.__doc__ = inventory.read_inventory.__doc__.replace("obspy", ns)
